...
# character class
class Character:
    def __init__(self, x, y, health, mob_animations, char_type, boss_enemy, size):
    ...
    def move(self, dx, dy, obstacle_tiles, exit_tile=None):
        ...
        if self.char_type == 0:
            ...
            if exit_tile[1].colliderect(self.rect):
                # check if the player is close to the center of the exit ladder
    ...
    # boundaries - OPTIONAL x(0,50), y(-20,90)
    def boundaries(self):
        self.rect.x = max(0, min(self.rect.x, constants.SCREEN_WIDTH - 50))
        self.rect.y = max(-20, min(self.rect.y, constants.SCREEN_HEIGHT - 90))
    ...
    def ai(self, player, obstacle_tiles,  screen_scroll, fireball_image):
        ...
        # check if line of sights passes through an obstacle tile
        for obstacle in obstacle_tiles:
        ...
        if self.alive:
            if not self.stunned:
                ...
                if self.boss_enemy and dist < 500:
    ...
    def update(self):
        ...
        if not self.running:
            self.update_action(0)  # 0:running
        else:
            self.update_action(1)  # 1:idle
    ...
    def draw(self, surface):
        ...
        if not self.char_type:
            ...
            surface.blit(flipped_image, (self.rect.x, (self.rect.y + 30) - constants.SCALE * constants.OFFSET))
        # pygame.draw.rect(surface, constants.RED, self.rect, 1)  # red rect of the character
